<x-app-layout>
    ...
    <section class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 bg-white shadow rounded mt-6">
        <h1 class="text-center text-xl font-semibold">Créer un film</h1>
        <form action="{{ route('films.store') }}" method="POST" class="mt-6">
            @csrf
            <div>
                <x-input-label>Titre</x-input-label>
                <x-text-input name="title"></x-text-input>
                <x-input-label>Année de sortie</x-input-label>
                <x-text-input name="release_year"></x-text-input>
            </div>
